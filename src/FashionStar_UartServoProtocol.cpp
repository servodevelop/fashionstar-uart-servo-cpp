...
void FSUS_Protocol::sendPack(){
    ...
    // 通过串口发送出去
    write(requestPack.header & 0xFF);
    write(requestPack.header >> 8);
    write(requestPack.cmdId);
    write(requestPack.content_size);
    for(uint16_t i=0; i<requestPack.content_size; i++){
        write(requestPack.content[i]);
    }
    write(requestPack.checksum);
}
